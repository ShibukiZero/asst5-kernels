...
// Entry 9 — CH=64 + uint16 (uchar2) vectorized load + lane-major conflict-free shared.
// Synthesis of the load-latency fix (vectorize, keep 32-lane warp -> preserve occupancy,
// unlike Entry 7's 128-thread crash) with a bank-conflict-free shared layout.
// Each lane reads ONE uint16 = 2 consecutive channels per load (64 bytes/warp/instr,
// 2x the byte-load version), unpacks 2 bytes, does 2 shared atomics.
// Shared layout is lane-major: slot = j*32 + lane, so s[v*CH + slot] has
// bank = (v*64 + j*32 + lane) % 32 = lane  -> conflict-free within a warp for every j.
// 64 KiB dynamic shared (opt-in). block=(32,32)=1024 threads; 2 blocks/SM fit (128 KiB).
#define CH     64
#define BINS   256
#define VEC    2
#define UNROLL 8
    ...
    int length, int num_channels, int num_bins)
{
    ...
    const int c0   = blockIdx.x * CH + lane * VEC; // first of this lane's 2 channels

    if (c0 < num_channels) {
        const int row0      = blockIdx.y * blockDim.y + threadIdx.y;
        const int rowStride = gridDim.y * blockDim.y;
        const bool has1     = (c0 + 1) < num_channels;
        ...
        for (; r + (UNROLL - 1) * rowStride < length; r += UNROLL * rowStride) {
            ...
            for (int k = 0; k < UNROLL; k++) {
                ...
                v[k] = *reinterpret_cast<const uint16_t*>(data + off);
            }
            #pragma unroll
            for (int k = 0; k < UNROLL; k++) {
                int v0 =  v[k]       & 0xff;
                ...
                if (has1) atomicAdd(&s[v1 * CH + 1 * 32 + lane], 1);
            }
        }
        for (; r < length; r += rowStride) {
            long long off = (long long)r * num_channels + c0;
            uint16_t pv = *reinterpret_cast<const uint16_t*>(data + off);
            int v0 =  pv       & 0xff;
            int v1 = (pv >> 8) & 0xff;
            atomicAdd(&s[v0 * CH + 0 * 32 + lane], 1);
            if (has1) atomicAdd(&s[v1 * CH + 1 * 32 + lane], 1);
...
torch::Tensor histogram_kernel(torch::Tensor data, int num_bins) {
    ...
    // uint16 vectorized load requires even num_channels (2-byte aligned offsets).
    TORCH_CHECK((num_channels % 2) == 0, "v8 requires even num_channels");
