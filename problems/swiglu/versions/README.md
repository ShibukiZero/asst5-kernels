...
| `v5_hybrid_h2.py` | 5 | 1.954 ms | CUTLASS gate + cuBLAS value + compiled epilogue + runtime guard. First hand kernel to beat baseline (~4% over Entry 2) |
...
| `v7a_h2_prealloc.py` | 7a | 1.948 ms | H2 + out-GEMM + cached buffers + direct fast path (Inductor epilogue). Prealloc alone barely helps — allocs are cheap; gap is launch overhead |
| **`v7b_h2_graph.py`** | 7b | **1.820 ms** | ✅ **BEST** — 7a + custom float4 fused epilogue + **CUDA Graph** capture/replay. Removes ~90µs launch/dispatch → wall ≈ GPU-kernel floor (1.81ms). Same H2 numerics, 6/6 seeds pass |
